  ...
  const renderCounselor = ({ item }) => {
    ...
    const onPayToChatButtonPressed = async () => {
      ...
      try {
        ...
        // --- Logic for successful 2xx responses ---
        // if (response?.data?.sessionNumber < 2) {
        ...
        //   });
        // } else {
        if (response?.data && response.data.expiredAt) {
          const expiredAtString = response.data.expiredAt;

          // 1. Parse the custom date string
          const parts = expiredAtString.split(', ');
          const dateParts = parts[0].split('/'); // [DD, MM, YYYY]
          const timeParts = parts[1].split(':'); // [HH, MM, SS]

          // Format for constructor: YYYY, MM (0-indexed), DD, HH, MM, SS
          const expiredAtDate = new Date(dateParts[2], dateParts[1] - 1, dateParts[0], timeParts[0], timeParts[1], timeParts[2]);

          // 2. Calculate the time difference
          const now = new Date();
          const timeDifferenceMillis = expiredAtDate.getTime() - now.getTime();
          const timeDifferenceMinutes = timeDifferenceMillis / (1000 * 60);

          // 3. Check if the session is still valid (less than 20 minutes have passed)
          if (timeDifferenceMinutes > 0) {
            ...
            if (response?.data?.sessionNumber >= 3) {
